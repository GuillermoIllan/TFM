...
library(plotly)
library(GeneOverlap)
setwd("~/Escritorio/datosrecuperados/scripts/Rscripts/AED/P0/P0_analisis/kallisto")
...
files <- paste("kallisto_analisis/general" ,  
               list.files(path = "kallisto_analisis/general",pattern = "abundance.tsv", recursive = TRUE),
               sep = "/")
...
dds <- DESeqDataSetFromTximport(txi.kallisto.tsv, coldata, ~Region +Tratamiento)
##PRE-FILTERING################################################################
dds
...
filesTissue <- paste("kallisto_analisis/Tissue" ,  
               list.files(path = "kallisto_analisis/Tissue",pattern = "abundance.tsv", recursive = TRUE),
               sep = "/")
names(filesTissue) <- rownames(controles_coldata)

txi.kallisto.tissue.tsv <- tximport(filesTissue, type = "kallisto", tx2gene = tx2gene, ignoreAfterBar = TRUE)
##Objeto DESeq #########################################
ddsC <- DESeqDataSetFromTximport(txi.kallisto.tissue.tsv, controles_coldata, ~Region)
ddsC
...
filesALBSF <- paste("kallisto_analisis/ALBSF_emb_VS_ctrl" ,  
                     list.files(path = "kallisto_analisis/ALBSF_emb_VS_ctrl",pattern = "abundance.tsv", recursive = TRUE),
                     sep = "/")
names(filesALBSF) <- rownames(ALBSF_coldata)

txi.kallisto.ALBSF.tsv <- tximport(filesALBSF, type = "kallisto", tx2gene = tx2gene, ignoreAfterBar = TRUE)
##Objeto DESeq##########################################
ddsA <- DESeqDataSetFromTximport(txi.kallisto.ALBSF.tsv, ALBSF_coldata, ~Tratamiento)
...
filesPMBSF <- paste("kallisto_analisis/PMBSF_emb_vs_ctrl" ,  
                    list.files(path = "kallisto_analisis/PMBSF_emb_vs_ctrl",pattern = "abundance.tsv", recursive = TRUE),
                    sep = "/")
names(filesPMBSF) <- rownames(PMBSF_coldata)

txi.kallisto.PMBSF.tsv <- tximport(filesPMBSF, type = "kallisto", tx2gene = tx2gene, ignoreAfterBar = TRUE)
##dds#######################################################
ddsP <- DESeqDataSetFromTximport(txi.kallisto.PMBSF.tsv, PMBSF_coldata, ~Tratamiento)
ddsP
...
saveRDS(genesPMBSF_ALBSF_emb_WPC, file = "../genes_overlapPMBSF-ALBSF_mbWPC_Kallisto.rds")
genesALBSF_ALBSF_emb_WPC<-genes_especificos$`ALBSF:ALBSF_mbWPC`
saveRDS(genesALBSF_ALBSF_emb_WPC, file = "../genes_overlapALBSF-ALBSF_mbWPC_Kallisto.rds")
...
filesoverlap <- paste("kallisto_analisis/overlap" ,  
                    list.files(path = "kallisto_analisis/overlap",pattern = "abundance.tsv", recursive = TRUE),
                    sep = "/")
names(filesoverlap) <- rownames(controles_coldata_vs_ALBSF_EMB_coldata)

txi.kallisto.overlap.tsv <- tximport(filesoverlap, type = "kallisto", tx2gene = cts_genes_overlap, ignoreAfterBar = TRUE)
#Creamos el objeto
dds_Spc_EmbWPC <- DESeqDataSetFromTximport(txi.kallisto.overlap.tsv, controles_coldata_vs_ALBSF_EMB_coldata, ~ Region + Tratamiento)
###prefiltering############################################################
